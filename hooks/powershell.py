...
def wait_for():
    wait_for_process(["powershell.exe"], hook)


def hook(pid):
    try:
        logger("Trying To Hook Into PowerShell")
        session = frida.attach(pid)
        logger(f"Hooked PowerShell With PID {pid}")
